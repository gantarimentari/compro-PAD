...
api.interceptors.request.use(
  (config) => {
    // 1. Ambil XSRF-TOKEN untuk jalur Cookie SPA (Localhost)
    ...
    // 🌟 2. AMBIL BEARER TOKEN DARI LOCALSTORAGE UNTUK JALUR VERCEL
    ...
    console.log('📤 API Request:', {
      ...
      data: config.data,
      headers: {
        'X-XSRF-TOKEN': xsrfToken ? '✓ Present' : '✗ Missing',
        'Authorization': bearerToken ? '✓ Present (Bearer)' : '✗ Missing', // Tambah log logistik
        'Accept': config.headers.Accept,
      }
    });
